...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <>
    <div className="w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto gap-4 relative pt-2">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "")}>
          <motion.div
            ...
            className={cn(
              ...
              selected?.id === card.id
                ? "rounded-lg cursor-pointer absolute inset-0 h-1/2 w-full md:w-2/3 lg:w-1/2 m-auto z-50 flex justify-center items-center flex-wrap flex-col"
  ...
  return (
    <div className="bg-transparent h-full w-full flex flex-col justify-end rounded-lg shadow-2xl relative z-[60]">
      ...
      <motion.div
        ...
        className="relative px-4 md:px-6 lg:px-8 pb-4 z-[70]"
      >
        {selected?.content}
      </motion.div>
    </div>
